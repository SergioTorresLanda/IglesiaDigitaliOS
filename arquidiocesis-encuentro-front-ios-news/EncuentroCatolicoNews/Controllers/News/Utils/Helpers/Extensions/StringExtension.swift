...
public extension String {
    var twoWords: String {
        get {
            ...
            if namesCount < 0 {
                ...
                return namesCount > 0 ? "\(splitName[0]) \(splitName[1])" : "\(splitName[0])"
            }
    ...
    func getStringFrom() -> String {
        let word = NSLocalizedString(self, tableName: nil, bundle: Bundle.init(for: InitViewViewController.self), value: "", comment: "")
        return word
    }
    
    func getImage() -> UIImage {
        return UIImage(named: self, in: Bundle.init(for: InitViewViewController.self), compatibleWith: nil) ?? UIImage()
    }
