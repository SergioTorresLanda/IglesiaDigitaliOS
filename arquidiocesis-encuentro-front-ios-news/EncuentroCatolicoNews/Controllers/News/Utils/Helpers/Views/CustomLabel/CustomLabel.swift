...
public typealias ConfigureLinkAttribute = (CustomLabelType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias ElementTuple = (range: NSRange, element: CustomLabelElement, type: CustomLabelType)
...
open class CustomLabel: UILabel {
    ...
    open var enabledTypes: [CustomLabelType] = [.mention, .hashtag, .url]
    ...
    open var customColor: [CustomLabelType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    
    open var customSelectedColor: [CustomLabelType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: CustomLabelType, handler: @escaping (String) -> ()) {
        customTapHandlers[type] = handler
    }
    
    open func removeHandle(for type: CustomLabelType) {
        switch type {
        case .hashtag:
    ...
    //MARK: - Customzation
    @discardableResult
    open func customize(_ block: (_ label: CustomLabel) -> ()) -> CustomLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [CustomLabelType : ((String) -> ())] = [:]
    ...
    lazy var activeElements = [CustomLabelType: [ElementTuple]]()
    ...
    fileprivate func parseTextAndExtractActiveElements(_ attrString: NSAttributedString) -> String {
        ...
        if enabledTypes.contains(.url) {
            let tuple = CustomLabelBuilder.createURLElements(from: textString, range: textRange, maximumLength: urlMaximumLength)
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = CustomLabelBuilder.createElements(type: type, from: textString, range: textRange, filterPredicate: filter)
    ...
    fileprivate func didTap(_ element: String, for type: CustomLabelType) {
...
extension CustomLabel: UIGestureRecognizerDelegate {
