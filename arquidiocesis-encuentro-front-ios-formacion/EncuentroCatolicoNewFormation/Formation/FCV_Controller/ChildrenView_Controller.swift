...
protocol FCV_VIPER_PresenterToViewProtocol: class {
    var _presenter : FCV_VIPER_ViewToPresenterProtocol? {set get}
    func setDataChild(data: [FF_Formation_Entity])
}

class ChildrenView_Controller: UIViewController {

    var searchField : UITextField?
    var tableChild: UITableView?
    ...
    var _presenter: FCV_VIPER_ViewToPresenterProtocol?
    ...
    override func viewDidLoad() {
        ...
        tableChild?.backgroundColor = colorBackground
        ...
        tableChild = UITableView(frame: CGRect.zero)
        guard let tableChild = tableChild else { debugPrint("Can't create tableNew"); return }
        tableChild.translatesAutoresizingMaskIntoConstraints = false
        tableChild.keyboardDismissMode = UIScrollView.KeyboardDismissMode.onDrag
        tableChild.delegate = self
        tableChild.dataSource = self
        tableChild.register(InfoView_Cell.self, forCellReuseIdentifier: "cell")
        tableChild.separatorColor = UIColor.clear
        tableChild.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 100, right: 0)

        
        let codeSegmented = CustomSegmentController(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 0), sgmTitles: ["PDF", "Videos"])
        ...
        self.view.addSubview(tableChild)
        ...
        NSLayoutConstraint.activate([
            ...
            tableChild.topAnchor.constraint(equalTo: searchField.bottomAnchor, constant: 118),
            tableChild.rightAnchor.constraint(equalTo: self.view.rightAnchor, constant: -17),
            tableChild.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: 17),
            tableChild.bottomAnchor.constraint(equalTo: self.view.bottomAnchor, constant: 40),
            ...
            imageFooter.leftAnchor.constraint(equalTo: self.view.leftAnchor, constant: (self.view.bounds.width/20)*6.2),
...
extension ChildrenView_Controller: FCV_VIPER_PresenterToViewProtocol {
    ...
    func setDataChild(data: [FF_Formation_Entity]){
        ...
        if arrFilterFile.count > 0 { arTitles?.append("PDF") }
        if arrFilterVideo.count > 0 { arTitles?.append("Videos") }
        if arrFilterLink.count > 0 { arTitles?.append("Enlances") }
...
extension ChildrenView_Controller: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !onSearch{
            //_presenter?.goNext(url: tableInfoNew?[indexPath.row].url ?? "")
           ...
           // _presenter?.goNext(url: tableFilterData[indexPath.row].url)
...
extension ChildrenView_Controller: UITextFieldDelegate {
    
    @objc func textFieldActive() {
        if onSearch{
            viewNoResult?.removeFromSuperview()
            tableChild?.isHidden = tableFilterData.count > 0 ? false : true
        }else{
            viewNoResult?.removeFromSuperview()
            let enable = tableFilterData.count > 0 ? true : false
            tableChild?.isHidden = enable
...
extension ChildrenView_Controller: CustomSegmentControllerDelegate{
